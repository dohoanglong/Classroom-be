...
class GradereviewController {
    ...
    static remove = async (req, res) => {
        try {
            await GradeReview.delete({
                where: { id: req.params.gradeReviewId },
            })
            res.status(200).send({ message: 'success' })
        } catch (error) {
            console.log(error)
    ...
    static get = async (req, res) => {
        try {
            const gradeReview = await GradeReview.getDetailById(
    ...
    static updateScoreAndStatus = async (req, res, next) => {
        try {
            ...
            if (!isTeacher) {
                res.status(200).send({
                    message: 'only teacher can view all grade review of class',
                })
                return
            }
            const { status, gradeReviewId, newScore } = req.body
            const gradeReview = await GradeReview.update(
                { status: status },
                {
    ...
    static addComment = async (req, res, next) => {
        try {
            ...
            req.userIdReceive = !isTeacher
                ? userCourse.teacherId
                : gradeReview.userId
            await NotificationController.add(req, res, next)
    ...
    static getAllOf = async (req, res) => {
        try {
            ...
            // });
            const gradeReviews = await GradeReview.getAllOf(
                req.params.courseId,
                req.user.id
            )
            res.status(200).send(gradeReviews)
