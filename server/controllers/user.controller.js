...
class user {
  static create = async (req, res, isFromSocial = false) => {
    try {
  ...
  // Find a single user
  static findOne = async (req, res, isFromSocial = false) => {
    try {
      const user = await User.findOne({ where: { mail: req.body.mail } });
      if (!isFromSocial) {
        if (!user)
  ...
  static authSocial = async (req, res) => {
    try {
      let data;
      if (req.body.fbToken) {
      ...
      if (data.email === req.body.mail) {
        ...
        if (!user) this.create(req, res, true);
        else this.findOne(req, res, true);
      } else {
        console.log(data.email, req.body.mail);
        res.status(400).send({
